/* catgrepmore.c by William Ho 
	Pipelines contents of file(s) into 'grep' which is then pipelined into 'more' */
...
main(int argc, char *argv[]) {
	int i, bytestowrite, byteswritten, status, infile;
	int bufsize = 2048, bufoffset, fds1[2], fds2[2];
	char *buf;
	pid_t morepid;
	sigset_t mask;
	...
	for (i=2; i<argc; i++) {
		...
		default: // cat
			...
			if (waitpid(morepid,&status,WUNTRACED) < 0) { // Wait for 'more' to exit before going to next file
